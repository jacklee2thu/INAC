...
temp_table<-data.frame(rep(sample_name[j],51120),summary_copy,bin_gc$gc)
...
temp_frame<-gastric_table[gastric_table$sample_name%in%sample_name,]
temp_frame$chromosome<-factor(temp_frame$chromosome,levels=c("chr1","chr2","chr3","chr4","chr5","chr6","chr7",
"chr8","chr9","chr10","chr11","chr12","chr13","chr14",
"chr15","chr16","chr17","chr18","chr19","chr20","chr21","chr22"))
library(ggplot2)
pdf('gastric_cancer_copy_num.pdf',width=50,height=10)
p <- ggplot(temp_frame, aes(x=bin, y=copy_num))+ylim(-1,1)
p + geom_point(alpha=0.8, shape=16,stat = "identity",position = "identity")+
theme_classic() + theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(),panel.margin = unit(0, "lines")) +
scale_x_continuous(expand = c(0, 0))+ facet_grid(~chromosome, scales = 'free_x', space = 'free_x', switch = 'x')
dev.off()
