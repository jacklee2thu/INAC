...
all_summary_table<-rbind(xiehe_table,gastric_table)
all_summary_table<-cbind(c(rep("healthy",dim(xiehe_table)[1]),rep("gastric",dim(gastric_table)[2])),all_summary_table)
...
gg <- ggplot(xiehe_table, aes(x=bin, y=ratio.center, group=sample_name)) + geom_line(size=0.001,color="#6EA6D9")+ylim(-.03,.05)
gg+theme_bw()+theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank(),
axis.title.x =element_text(size=20), axis.title.y=element_text(size=20),axis.text.x=element_text(size=20),
axis.text.y=element_text(size=20),legend.text=element_text(size=20))+labs(title = "fragment ratio.center of healthy controls")
dev.off()
