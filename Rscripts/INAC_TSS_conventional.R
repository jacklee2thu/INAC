...
tss_bed_up3000_1000<-as.character(argv[5])
tss_bed_down1000_3000<-as.character(argv[6])
tss_bed_up1000_down1000<-as.character(argv[7])
tss_bed_up150_down50<-as.character(argv[8])
...
patient_tss150<-list()
for(i in 1:dim(tss_bed_up1000_down1000)[1]){
...
temp_up1000_down1000[,4]<-temp_up1000_down1000[,3]
colnames(temp_up1000_down1000)<-c('chr','site','coverage')
}
...
temp_up150_down50[,4]<-temp_up150_down50[,3]
colnames(temp_up150_down50)<-c('chr','site','coverage')
}
...
tss_1000[i]<-mean(patient_tss1000[[i]][,3])
}else{tss_1000[i]<-NA}
if(!is.na(patient_tss150[[i]])){
tss_150[i]<-mean(patient_tss150[[i]][,3])
}else{tss_150[i]<-NA}
}
conventional_relative_coverage<-tss_150/tss_1000
colnames(tss_relative_coverage)<-c("tss_conventional")
all_sample_tss[[j]]<-tss_relative_coverage
}


###########
temp_tss_conventional<-all_sample_tss[[1]][,1]
save(temp_tss_conventional,file=paste0(sample_name,'_tss_conventional.Rdata'))
